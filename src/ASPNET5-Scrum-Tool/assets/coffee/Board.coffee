...
SubmitColumnForm = () ->
    $('.panel-heading').on 'click', 'input.ColumnTitleSubmit', (event) ->
        ...
        $.ajax
            ...
            type: 'POST',
            data: ***REMOVED***ColumnName: columnName, ColumnNumber: columnNumber ***REMOVED***,
            dataType: 'json',
            success: (data) ->              
                     #alert 'data is' + data
...
AddColumn = () ->
    $('#AddColumnButton').on 'click', (event) ->
        ...
        $.ajax
            url:'/Board/AddColumn',
            type: 'POST',
            data: ***REMOVED***ColumnName: newColumnName, ColumnID: newColumnDataID ***REMOVED***,
            success: (data) ->
...
SubmitTaskForm = () ->
     $('.panel-body').on 'click', '.TaskFormSubmit', (event) ->
          ...
          $.ajax
            ...
            type: 'POST'
            data: ***REMOVED***ParentColumn: selectedColumn, TaskID : taskID, TaskContent: taskContent ***REMOVED***
            success: (data) ->
                alert "Hit the success part"
                $('#AddColumnButton').before data
            error : (error) ->
