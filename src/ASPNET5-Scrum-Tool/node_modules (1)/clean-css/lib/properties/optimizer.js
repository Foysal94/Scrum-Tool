...
function _optimize(properties, mergeAdjacent, aggressiveMerging, validator) ***REMOVED***
  var overrideMapping = ***REMOVED******REMOVED***;
  var lastName = null;
  var lastProperty;
  var j;

  function mergeablePosition(position) ***REMOVED***
  ...
  ***REMOVED***
  ...
  function sameValue(position) ***REMOVED***
  ...
  ***REMOVED***
  ...
  propertyLoop:
  for (var position = 0, total = properties.length; position < total; position++) ***REMOVED***
    ...
    if (position > 0 && lastProperty && _name == lastName && isImportant == lastProperty.important && isHack == lastProperty.hack && sameValue(position) && !lastProperty.unused) ***REMOVED***
...
***REMOVED***
    ...
    // e.g. a***REMOVED***display:inline-block;display:-moz-inline-box***REMOVED***
    // however if `mergeablePosition` yields true then the rule does not apply
    // (e.g merging two adjacent selectors: `a***REMOVED***display:block***REMOVED***a***REMOVED***display:block***REMOVED***`)
    if (_name in overrideMapping && (aggressiveMerging && _name != lastName || mergeablePosition(position))) ***REMOVED***
      var toOverridePositions = overrideMapping[_name];
      var canOverride = compactable[_name] && compactable[_name].canOverride;
      var anyRemoved = false;

      for (j = toOverridePositions.length - 1; j >= 0; j--) ***REMOVED***
        ...
        if (wasImportant && !isImportant || wasImportant && isHack) ***REMOVED***
          property.unused = true;
...
***REMOVED***
  ***REMOVED***
      ...
      if (anyRemoved) ***REMOVED***
        ...
        lastProperty = null;
        lastName = null;
        overrideMapping = ***REMOVED******REMOVED***;
        continue;
  ***REMOVED***
***REMOVED*** else ***REMOVED***
      ...
      if (_shorthands) ***REMOVED***
        for (j = _shorthands.length - 1; j >= 0; j--) ***REMOVED***
...
function optimize(selector, properties, mergeAdjacent, withCompacting, options, validator) ***REMOVED***
  var _properties = wrapForOptimizing(properties);
  populateComponents(_properties, validator);
  _optimize(_properties, mergeAdjacent, options.aggressiveMerging, validator);

  for (var i = 0, l = _properties.length; i < l; i++) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (withCompacting && options.shorthandCompacting) ***REMOVED***
