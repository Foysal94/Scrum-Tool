...
__dirname="$(CDPATH= cd "$(dirname "$***REMOVED***BASH_SOURCE[0]***REMOVED***")" && pwd)"
dot_node_gyp=$***REMOVED***__dirname***REMOVED***/.node-gyp/
...
run_tests() ***REMOVED***
  ...
  run_cmd="rsync -aAXx --delete --exclude .git --exclude build /node-gyp-src/ /usr/lib/node_modules/npm/node_modules/node-gyp/;
    /bin/su -s /bin/bash node-gyp -c 'cd && $***REMOVED***run_cmd***REMOVED***'"
  ...
  docker run \
    --rm -i \
    -v ~/.npm/:/node-gyp/.npm/ \
    -v $***REMOVED***dot_node_gyp***REMOVED***:/node-gyp/.node-gyp/ \
    -v $(pwd):/node-gyp-src/:ro \
    node-gyp-test/$***REMOVED***version***REMOVED*** /bin/bash -c "$***REMOVED***run_cmd***REMOVED***"
***REMOVED***
...
for v in $test_node_versions; do
  setup_container "node-gyp-test/$***REMOVED***v***REMOVED***" "node-gyp-test/clones" "
    curl -sL https://nodejs.org/dist/v$***REMOVED***v***REMOVED***/node-v$***REMOVED***v***REMOVED***-linux-x64.tar.gz | tar -zxv --strip-components=1 -C /usr/ &&
...
for v in $test_iojs_versions; do
  setup_container "node-gyp-test/$***REMOVED***v***REMOVED***" "node-gyp-test/clones" "
    curl -sL https://iojs.org/dist/v$***REMOVED***v***REMOVED***/iojs-v$***REMOVED***v***REMOVED***-linux-x64.tar.gz | tar -zxv --strip-components=1 -C /usr/ &&
    npm install npm@latest -g &&
    node -v && npm -v
  "
done
...
test_download_node_version() ***REMOVED***
  local run_with_ver="$1"
  local expected_dir="$2"
  local expected_ver="$3"
  run_tests $run_with_ver "cd node-buffertools && npm install --loglevel=info --target=$***REMOVED***expected_ver***REMOVED***"
  local node_ver=$(cat "$***REMOVED***dot_node_gyp***REMOVED***$***REMOVED***expected_dir***REMOVED***/node_version.h" | grep '#define NODE_\w*_VERSION [0-9]*$')
  node_ver=$(echo $node_ver | sed 's/#define NODE_[A-Z]*_VERSION //g' | sed 's/ /./g')
  if [ "X$(echo $node_ver)" != "X$***REMOVED***expected_ver***REMOVED***" ]; then
    echo "Did not download v$***REMOVED***expected_ver***REMOVED*** using --target, instead got: $(echo $node_ver)"
    exit 1
  fi
  echo "Verified correct download of [v$***REMOVED***node_ver***REMOVED***]"
***REMOVED***
