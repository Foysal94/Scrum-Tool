...
***REMOVED***
  ...
  std::vector<void*> argv;
  for (unsigned l = sass_list_get_length(s_args), i = 0; i < l; i++) ***REMOVED***
...
int ExtractOptions(v8::Local<v8::Object> options, void* cptr, sass_context_wrapper* ctx_w, bool is_file, bool is_sync) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (importer_callback->IsFunction()) ***REMOVED***
  ...
  ***REMOVED***
  else if (importer_callback->IsArray()) ***REMOVED***
    ...
    for (size_t i = 0; i < importers->Length(); ++i) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  if (custom_functions->IsObject()) ***REMOVED***
    ...
    for (unsigned i = 0; i < num_signatures; i++) ***REMOVED***
...
void GetStats(sass_context_wrapper* ctx_w, Sass_Context* ctx) ***REMOVED***
  Nan::HandleScope scope;

  char** included_files = sass_context_get_included_files(ctx);
  v8::Local<v8::Array> arr = Nan::New<v8::Array>();

  if (included_files) ***REMOVED***
    for (int i = 0; included_files[i] != nullptr; ++i) ***REMOVED***
...
int GetResult(sass_context_wrapper* ctx_w, Sass_Context* ctx, bool is_sync = false) ***REMOVED***
  Nan::HandleScope scope;
...
void MakeCallback(uv_work_t* req) ***REMOVED***
  Nan::HandleScope scope;
  ...
  ***REMOVED***
  ...
  if (status == 0 && ctx_w->success_callback) ***REMOVED***
    // if no error, do callback(null, result)
    ctx_w->success_callback->Call(0, 0);
  ***REMOVED***
  else if (ctx_w->error_callback) ***REMOVED***
...
***REMOVED***;
  ...
  ***REMOVED***
  if (try_catch.HasCaught()) ***REMOVED***
...
NAN_METHOD(render) ***REMOVED***
  ...
  if (ExtractOptions(options, dctx, ctx_w, false, false) >= 0) ***REMOVED*** 
...
NAN_METHOD(render_sync) ***REMOVED***
  ...
  if ((result = ExtractOptions(options, dctx, ctx_w, false, true)) >= 0) ***REMOVED***
    compile_data(dctx);
...
NAN_METHOD(render_file) ***REMOVED***
  ...
  if (ExtractOptions(options, fctx, ctx_w, true, false) >= 0) ***REMOVED***
...
NAN_METHOD(render_file_sync) ***REMOVED***
  ...
  if ((result = ExtractOptions(options, fctx, ctx_w, true, true)) >= 0) ***REMOVED***
    compile_file(fctx);
    result = GetResult(ctx_w, ctx, true);
  ***REMOVED***;
...
NAN_METHOD(libsass_version) ***REMOVED***
  info.GetReturnValue().Set(Nan::New<v8::String>(libsass_version()).ToLocalChecked());
***REMOVED***

NAN_MODULE_INIT(RegisterModule) ***REMOVED***
