...
template <typename T, typename L>
CallbackBridge<T, L>::CallbackBridge(v8::Local<v8::Function> callback, bool is_sync) : callback(new Nan::Callback(callback)), is_sync(is_sync) ***REMOVED***
  /* 
...
template <typename T, typename L>
CallbackBridge<T, L>::~CallbackBridge() ***REMOVED***
...
template <typename T, typename L>
T CallbackBridge<T, L>::operator()(std::vector<void*> argv) ***REMOVED***
  // argv.push_back(wrapper);
  if (this->is_sync) ***REMOVED***
    /* 
     * This is invoked from the main JavaScript thread.
     * V8 context is available.
     *
...
***REMOVED***
  ...
  ***REMOVED*** else ***REMOVED***
    ...
    while (!this->has_returned) ***REMOVED***
...
template <typename T, typename L>
void CallbackBridge<T, L>::dispatched_async_uv_callback(uv_async_t *req) ***REMOVED***
...
template <typename T, typename L>
NAN_METHOD(CallbackBridge<T COMMA L>::ReturnCallback) ***REMOVED***
...
template <typename T, typename L>
Nan::MaybeLocal<v8::Function> CallbackBridge<T, L>::get_wrapper_constructor() ***REMOVED***
  /* Uses handle scope created in the CallbackBridge<T, L> constructor */
  if (wrapper_constructor.IsEmpty()) ***REMOVED***
...
template <typename T, typename L>
NAN_METHOD(CallbackBridge<T COMMA L>::New) ***REMOVED***
  info.GetReturnValue().Set(info.This());
***REMOVED***

template <typename T, typename L>
void CallbackBridge<T, L>::async_gone(uv_handle_t *handle) ***REMOVED***
