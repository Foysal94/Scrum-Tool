...
install:
  ...
  - ps: |
      if(!(gem which minitest 2>$nul)) ***REMOVED*** gem install minitest --no-ri --no-rdoc ***REMOVED***
      if ($env:Compiler -eq "mingw" -AND -Not (Test-Path "C:\mingw64")) ***REMOVED***
        # Install MinGW.
...
build_script:
  - ps: |
      if ($env:Compiler -eq "mingw") ***REMOVED***
