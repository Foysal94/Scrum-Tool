...
namespace Sass ***REMOVED***
  ...
  ***REMOVED*** return sass_importer_get_priority(i) > sass_importer_get_priority(j); ***REMOVED***
  ...
  ***REMOVED***
    ...
    if (safe_path == "" && input_path != "") ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (size_t i = 0; i < resources.size(); ++i) ***REMOVED***
...
***REMOVED***
    ...
    for (size_t m = 0; m < import_stack.size(); ++m) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    if (paths_str) ***REMOVED***
      const char* beg = paths_str;
      const char* end = Prelexer::find_first<PATH_SEP>(beg);

      while (end) ***REMOVED***
        std::string path(beg, end - beg);
        if (!path.empty()) ***REMOVED***
          if (*path.rbegin() != '/') path += '/';
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (size_t i = 0; paths_array[i]; i++) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    // if (sheets.count(inc.abs_path)) ***REMOVED***
    ...
    std::pair<const std::string, const StyleSheet>
      ast_pair(inc.abs_path, ***REMOVED*** res, root ***REMOVED***);
...
***REMOVED***
    ...
    // process the resolved entry
      ...
      if (char* contents = read_file(resolved[0].abs_path)) ***REMOVED***
        // register the newly resolved file resource
        register_resource(resolved[0], ***REMOVED*** contents, 0 ***REMOVED***);
...
***REMOVED***
    ...
    // nothing found
    return ***REMOVED*** imp, "" ***REMOVED***;

  ***REMOVED***

  void Context::import_url (Import* imp, std::string load_path, const std::string& ctx_path) ***REMOVED***
    ...
    if (const char* proto = sequence< identifier, exactly<':'>, exactly<'/'>, exactly<'/'> >(imp_path.c_str())) ***REMOVED***

      protocol = std::string(imp_path.c_str(), proto - 3);
      // std::cerr << "==================== " << protocol << "\n";
      if (protocol.compare("file") && true) ***REMOVED***
...
***REMOVED***
    ...
    if (imp->media_queries() || protocol != "file" || imp_path.substr(0, 2) == "//") ***REMOVED***
      imp->urls().push_back(SASS_MEMORY_NEW(mem, String_Quoted, imp->pstate(), load_path));
***REMOVED***
    else if (imp_path.length() > 4 && imp_path.substr(imp_path.length() - 4, 4) == ".css") ***REMOVED***
...
***REMOVED***
      ...
      if (include.abs_path.empty()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (Sass_Importer_Entry& importer : importers) ***REMOVED***
  ...
  ***REMOVED***
          ...
          if (const char* err_message = sass_import_get_error_message(include)) ***REMOVED***
            if (source || srcmap) register_resource(***REMOVED*** importer, uniq_path ***REMOVED***, ***REMOVED*** source, srcmap ***REMOVED***);
  ...
  ***REMOVED***
          // content for import was set
            ...
            register_resource(include, ***REMOVED*** source, srcmap ***REMOVED***);
  ***REMOVED***
          // only a path was retuned
          // try to load it like normal
          else if(abs_path) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (size_t i = 0, S = imp->incs().size(); i < S; ++i) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (size_t i = 0, S = include_paths.size(); contents == 0 && i < S; ++i) ***REMOVED***
...
***REMOVED***
    ...
    register_resource(***REMOVED******REMOVED*** input_path, "." ***REMOVED***, abs_path ***REMOVED***, ***REMOVED*** contents, 0 ***REMOVED***);
  ...
  ***REMOVED***
    ...
    if(c_options->is_indented_syntax_src) ***REMOVED***
...
***REMOVED***
    ...
    register_resource(***REMOVED******REMOVED*** input_path, "." ***REMOVED***, input_path ***REMOVED***, ***REMOVED*** source_c_str, srcmap_c_str ***REMOVED***);
...
***REMOVED*** register_c_function(*this, &global, c_functions[i]); ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
      ...
      if (skip) ***REMOVED*** includes.erase( includes.begin(), includes.begin() + 1 + headers); ***REMOVED***
      else ***REMOVED*** includes.erase( includes.begin() + 1, includes.begin() + 1 + headers); ***REMOVED***
