...
***REMOVED***
    ...
    if (r->is_root()) ***REMOVED***
      if (Selector_List* selector_list = dynamic_cast<Selector_List*>(r->selector())) ***REMOVED***
        for (Complex_Selector* complex_selector : selector_list->elements()) ***REMOVED***
          Complex_Selector* tail = complex_selector;
          while (tail) ***REMOVED***
            if (tail->head()) for (Simple_Selector* header : tail->head()->elements()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    for (size_t i = 0, L = expanded_block->length(); i < L; ++i) ***REMOVED***
      Statement* stm = (*expanded_block)[i];
      if (Declaration* dec = static_cast<Declaration*>(stm)) ***REMOVED***
        String_Schema* combined_prop = SASS_MEMORY_NEW(ctx.mem, String_Schema, p->pstate());
        if (!property_stack.empty()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    if (a->is_global()) ***REMOVED***
      if (a->is_default()) ***REMOVED***
        if (env->has_global(var)) ***REMOVED***
          Expression* e = dynamic_cast<Expression*>(env->get_global(var));
          if (!e || e->concrete_type() == Expression::NULL_VAL) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
***REMOVED***
    else if (a->is_default()) ***REMOVED***
      if (env->has_lexical(var)) ***REMOVED***
        auto cur = env;
        while (cur && cur->is_lexical()) ***REMOVED***
          if (cur->has_local(var)) ***REMOVED***
            if (AST_Node* node = cur->get_local(var)) ***REMOVED***
...
***REMOVED***
        throw std::runtime_error("Env not in sync");
  ***REMOVED***
      else if (env->has_global(var)) ***REMOVED***
        if (AST_Node* node = env->get_global(var)) ***REMOVED***
          Expression* e = dynamic_cast<Expression*>(node);
          if (!e || e->concrete_type() == Expression::NULL_VAL) ***REMOVED***
            env->set_global(var, a->value()->perform(&eval));
  ***REMOVED***
***REMOVED***
  ***REMOVED***
      else if (env->is_lexical()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    if (*i->predicate()->perform(&eval)) ***REMOVED***
...
***REMOVED***
    ...
    if (sass_start->unit() != sass_end->unit()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    List* list = 0;
    Map* map = 0;
    if (expr->concrete_type() == Expression::MAP) ***REMOVED***
      map = static_cast<Map*>(expr);
***REMOVED***
    else if (expr->concrete_type() != Expression::LIST) ***REMOVED***
...
***REMOVED***
    ...
    for (size_t i = 0, L = variables.size(); i < L; ++i) ***REMOVED***
...
***REMOVED***
      ...
      for (size_t i = 0, L = list->length(); i < L; ++i) ***REMOVED***
        Expression* e = (*list)[i];
        // unwrap value if the expression is an argument
        if (Argument* arg = dynamic_cast<Argument*>(e)) e = arg->value();
        // check if we got passed a list of args (investigate)
        if (List* scalars = dynamic_cast<List*>(e)) ***REMOVED***
          if (variables.size() == 1) ***REMOVED***
            Expression* var = scalars;
            if (arglist) var = (*scalars)[0];
            env->set_local(variables[0], var);
  ***REMOVED*** else ***REMOVED***
            for (size_t j = 0, K = variables.size(); j < K; ++j) ***REMOVED***
...
***REMOVED***
  ***REMOVED***
***REMOVED*** else ***REMOVED***
          if (variables.size() > 0) ***REMOVED***
            env->set_local(variables[0], e);
            for (size_t j = 1, K = variables.size(); j < K; ++j) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    Block* body = w->block();
    while (*pred->perform(&eval)) ***REMOVED***
      append_block(body);
***REMOVED***
    return 0;
  ***REMOVED***
  ...
  ***REMOVED***
    ...
    if (Selector_List* selector_list = dynamic_cast<Selector_List*>(e->selector())) ***REMOVED***
      for (Complex_Selector* complex_selector : selector_list->elements()) ***REMOVED***
        Complex_Selector* tail = complex_selector;
        while (tail) ***REMOVED***
          if (tail->head()) for (Simple_Selector* header : tail->head()->elements()) ***REMOVED***
...
***REMOVED***
    ...
    for (auto complex_sel : contextualized->elements()) ***REMOVED***
      Complex_Selector* c = complex_sel;
      if (!c->head() || c->tail()) ***REMOVED***
...
***REMOVED***
    ...
    if (c->block() && c->name() != "@content" && !body->has_content()) ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    if (block_stack.back()->is_root()) ***REMOVED***
      selector_stack.push_back(0);
***REMOVED***
