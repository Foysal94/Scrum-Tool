...
namespace Sass ***REMOVED***
  namespace Prelexer ***REMOVED***
    ...
    // Regex equivalent: /(?:x)/
    template <char chr>
    const char* exactly(const char* src) ***REMOVED***
...
***REMOVED***
    ...
    template <const char* str>
    const char* insensitive(const char* src) ***REMOVED***
      if (str == 0) return 0;
      const char* pre = str;
      if (src == 0) return 0;
      // there is a small chance that the search string
      // is longer than the rest of the string to look at
      while (*pre && (*src == *pre || *src+32 == *pre)) ***REMOVED***
        ++src, ++pre;
  ***REMOVED***
      // did the matcher finish?
      return *pre == 0 ? src : 0;
***REMOVED***
    ...
    const char* class_char(const char* src) ***REMOVED***
...
***REMOVED***
    ...
    // Regex equivalent: /[axy]+/
    template <const char* char_class>
    const char* class_chars(const char* src) ***REMOVED***
      const char* p = src;
...
***REMOVED***
    ...
    const char* any_char_but(const char* src) ***REMOVED***
...
***REMOVED***
    ...
    const char* negate(const char* src) ***REMOVED***
      return mx(src) ? 0 : src;
***REMOVED***
    ...
    template <prelexer mx>
    const char* lookahead(const char* src) ***REMOVED***
      return mx(src) ? src : 0;
***REMOVED***
    ...
    const char* alternatives(const char* src) ***REMOVED***
      const char* rslt;
...
***REMOVED***
    ...
    const char* sequence(const char* src) ***REMOVED***
      const char* rslt = src;
...
***REMOVED***
    ...
    // Regex equivalent: /(?:literal)?/
    template <prelexer mx>
    const char* optional(const char* src) ***REMOVED***
      const char* p = mx(src);
      return p ? p : src;
***REMOVED***
    ...
    // Regex equivalent: /(?:literal)*/
    template <prelexer mx>
    const char* zero_plus(const char* src) ***REMOVED***
      const char* p = mx(src);
...
***REMOVED***
    ...
    // Regex equivalent: /(?:literal)+/
    template <prelexer mx>
    const char* one_plus(const char* src) ***REMOVED***
      const char* p = mx(src);
      if (!p) return 0;
      while (p) src = p, p = mx(src);
      return src;
***REMOVED***
    ...
    const char* non_greedy(const char* src) ***REMOVED***
      while (!delim(src)) ***REMOVED***
        const char* p = mx(src);
        if (p == src) return 0;
        if (p == 0) return 0;
        src = p;
...
***REMOVED***
    ...
    template <const char* str>
    const char* keyword(const char* src) ***REMOVED***
      return sequence <
               insensitive < str >,
               word_boundary
             >(src);
***REMOVED***
    ...
    template <const char* str>
    const char* word(const char* src) ***REMOVED***
      return sequence <
               exactly < str >,
               word_boundary
             >(src);
***REMOVED***
    ...
    template <char chr>
    const char* loosely(const char* src) ***REMOVED***
      return sequence <
               optional_spaces,
               exactly < chr >
             >(src);
