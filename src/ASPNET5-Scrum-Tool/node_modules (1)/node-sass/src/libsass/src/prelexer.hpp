...
namespace Sass ***REMOVED***
  ...
  namespace Prelexer ***REMOVED***
    ...
    const char* delimited_by(const char* src) ***REMOVED***
      src = exactly<beg>(src);
      if (!src) return 0;
      const char* stop;
      while (1) ***REMOVED***
...
***REMOVED***
    ...
    const char* skip_over_scopes(const char* src, const char* end) ***REMOVED***
...
***REMOVED***
    ...
    // first start/opener must be consumed already!
    template<prelexer start, prelexer stop>
    const char* skip_over_scopes(const char* src) ***REMOVED***
      return skip_over_scopes<start, stop>(src, 0);
***REMOVED***

    // Match a sequence of characters delimited by the supplied chars.
    template <prelexer start, prelexer stop>
    const char* recursive_scopes(const char* src) ***REMOVED***
      // parse opener
      src = start(src);
      // abort if not found
      if (!src) return 0;
...
***REMOVED***
    ...
    const char* between(const char* src) ***REMOVED***
      for (size_t i = 0; i < lo; ++i) ***REMOVED***
        src = mx(src);
        if (!src) return 0;
  ***REMOVED***
      for (size_t i = lo; i <= hi; ++i) ***REMOVED***
...
***REMOVED***
    ...
    template<char c>
    const char* find_first(const char* src) ***REMOVED***
      while (*src && *src != c) ++src;
      return *src ? src : 0;
***REMOVED***
...
***REMOVED***
    template<prelexer mx>
    const char* find_first_in_interval(const char* beg, const char* end) ***REMOVED***
      bool esc = false;
      while ((beg < end) && *beg) ***REMOVED***
        if (esc) esc = false;
...
***REMOVED***
    template <prelexer mx>
    unsigned int count_interval(const char* beg, const char* end) ***REMOVED***
      ...
      while (beg < end && *beg) ***REMOVED***
        ...
        if (esc) ***REMOVED***
          esc = false;
          ++beg;
***REMOVED*** else if (*beg == '\\') ***REMOVED***
          esc = true;
          ++beg;
***REMOVED*** else if ((p = mx(beg))) ***REMOVED***
          ++counter;
          beg = p;
***REMOVED***
        else ***REMOVED***
