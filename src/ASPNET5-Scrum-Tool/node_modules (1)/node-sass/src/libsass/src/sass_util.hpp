...
namespace Sass ***REMOVED***
  ...
  */
  class DefaultLcsComparator ***REMOVED***
  public:
    bool operator()(const Node& one, const Node& two, Node& out) const ***REMOVED***
      // TODO: Is this the correct C++ interpretation?
      // block ||= proc ***REMOVED***|a, b| a == b && a***REMOVED***
      if (one == two) ***REMOVED***
        out = one;
        return true;
  ***REMOVED***
...
***REMOVED***
  ...
  */
  ...
  Node lcs_backtrace(const LCSTable& c, const Node& x, const Node& y, int i, int j, const ComparatorType& comparator) ***REMOVED***
...
***REMOVED***
    ...
    if (comparator(xChildren[i], yChildren[j], compareOut)) ***REMOVED***
...
***REMOVED***
  ...
  */
  template<typename ComparatorType>
  void lcs_table(const Node& x, const Node& y, const ComparatorType& comparator, LCSTable& out) ***REMOVED***
    ...
    // x.size.times ***REMOVED***|i| c[i][0] = 0***REMOVED***
    // y.size.times ***REMOVED***|j| c[0][j] = 0***REMOVED***

    for (size_t i = 1; i < xChildren.size(); i++) ***REMOVED***
      for (size_t j = 1; j < yChildren.size(); j++) ***REMOVED***
        Node compareOut = Node::createNil();
...
***REMOVED***
  ...
  */
  template<typename ComparatorType>
  Node lcs(Node& x, Node& y, const ComparatorType& comparator, Context& ctx) ***REMOVED***
  ...
  */
  ...
  void group_by_to_a(std::vector<EnumType>& enumeration, KeyFunctorType& keyFunc, std::vector<std::pair<KeyType, std::vector<EnumType> > >& arr /*out*/) ***REMOVED***

    std::map<unsigned int, KeyType> order;

    std::map<KeyType, std::vector<EnumType> > grouped;

    for (typename std::vector<EnumType>::iterator enumIter = enumeration.begin(), enumIterEnd = enumeration.end(); enumIter != enumIterEnd; enumIter++) ***REMOVED***
      EnumType& e = *enumIter;

      KeyType key = keyFunc(e);

      if (grouped.find(key) == grouped.end()) ***REMOVED***
...
***REMOVED***
    ...
    for (unsigned int index = 0; index < order.size(); index++) ***REMOVED***
